...
png(here("figures", "C_regression_plot.png"), width=1200, height=1100, res=216)
par(mar=c(5,5,4,2))
...
png(here("figures", "NH_CWmeanC_RegressionPlot2.png"), width=1200, height=1100, res=216)
par(mar=c(5,5,4,2))
...
png(here::here("figures", "MEboxplot_2panel_alltypes_pub.png"), width=1300, height=1000, res=216)
newMeanC.plot<-ggboxplot(FQA.data_rank, x = "EIA_RANK_NOSIZE", y = "avCOC", size=1, color="black" )+ 
  ylim(1,8)+
  ggtitle("New")+
...
oldMeanC.plot<-ggboxplot(FQA.data_rank, x = "EIA_RANK_NOSIZE", y = "OLDavCOC", size=1, color="darkgray" )+ 
  ylim(1,8)+
  ggtitle("Old")+
  labs(x = "EIA Rank", y="Mean-C")+
  font("xylab", size = 16, face="bold")+
...
png('C:/data/MEboxplot_all_CWmeanC_pub2.png', width=1300, height=1000, res=216)
...
#three panel plot for publicatoin
tiff('C:/data/Maine_pub_Fig2_4_9_2019.tiff', width=3000, height=1000, res=300)

par(mfrow=c(1,3), mar=c(5,5,1,1))
#panel A community weighted COC
cwCOC_regression_cwCOC <- lm(FQA.data$cwCOC ~ FQA.data$EIA_score_NOSIZE)
OLDcwCOC_regression_cwCOC <- lm(FQA.data$OLDcwCOC ~ FQA.data$EIA_score_NOSIZE)
newCW.summary<-summary(cwCOC_regression_cwCOC)
newCW.summary$r.squared
...
plot(FQA.data$cwCOC ~ FQA.data$EIA_score_NOSIZE, 
     xlab = "EIA score", ylab = "CW-Mean C", xlim=c(1, 4.1), ylim=c(1,7.5), pch=21, col='black', bg='black', cex=1.2, cex.lab=1.5, font.lab=2, font.axis=2)
abline(cwCOC_regression_cwCOC, lwd=2, col='black')

points(FQA.data$OLDcwCOC ~ FQA.data$EIA_score_NOSIZE, pch=21, cex=1.2, col="black", bg='gray')
abline(OLDcwCOC_regression_cwCOC, lty=5, lwd=2, col ='gray')

legend("topleft", title=expression(underline("All Types")), legend=c(expression(paste("New: ", "R"^"2"," =0.26, p < 0.001")), expression(paste("Old: ", "R"^"2"," = 0.19, p < 0.001"))), bty="n",
       col= c('black', 'black'), pt.bg=c('black','gray'), pch=c(21, 21), cex=1, text.font= c(2,1,1))

legend("topright", legend ="A", bty="n", cex= 2, text.font= c(2,4,2))


#Panel B Floodplain community weighted COC
cwCOC_regression_cwCOC <- lm(FQA.flood$cwCOC ~ FQA.flood$EIA_score_NOSIZE)
OLDcwCOC_regression_cwCOC <- lm(FQA.flood$OLDcwCOC ~ FQA.flood$EIA_score_NOSIZE)
newCW.summary<-summary(cwCOC_regression_cwCOC)
newCW.summary$r.squared
...
points(FQA.flood$OLDcwCOC ~ FQA.flood$EIA_score_NOSIZE, pch=21, cex=1.2, col="black", bg='gray')
abline(OLDcwCOC_regression_cwCOC, lty=5, lwd=2, col ='gray')

legend("topleft", title=expression(underline("Large River Floodplain")), legend=c(expression(paste("New: ", "R"^"2"," =0.17, p < 0.001")), expression(paste("Old: ", "R"^"2"," = 0.11, p < 0.001"))), bty="n",
       col= c('black', 'black'), pt.bg=c('black','gray'), pch=c(21, 21), cex=1, text.font= c(2,1,1))

legend("topright", legend ="B", bty="n", cex= 2, text.font= c(2,4,2))

##Panel C swamp community weighted COC
cwCOC_regression_cwCOC <- lm(FQA.swamp$cwCOC ~ FQA.swamp$EIA_score_NOSIZE)
OLDcwCOC_regression_cwCOC <- lm(FQA.swamp$OLDcwCOC ~ FQA.swamp$EIA_score_NOSIZE)
newCW.summary<-summary(cwCOC_regression_cwCOC)
newCW.summary$r.squared
...
points(FQA.swamp$OLDcwCOC ~ FQA.swamp$EIA_score_NOSIZE, pch=21, cex=1.2, col="black", bg='gray')
abline(OLDcwCOC_regression_cwCOC, lty=5, lwd=2, col ='gray')

legend("topleft", title=expression(underline("Northern Swamp")), legend=c(expression(paste("New: ", "R"^"2"," =0.49, p < 0.001")), expression(paste("Old: ", "R"^"2"," = 0.52, p < 0.001"))), bty="n",
       col= c('black', 'black'), pt.bg=c('black','gray'), pch=c(21, 21), cex=1, text.font= c(2,1,1))
legend("topright", legend ="C", bty="n", cex= 2, text.font= c(2,4,2))
dev.off()
...
#PartC
png('C:/data/combo_box_plts.png', width=1300, height=1000, res=216)
...
ggarrange(oldMeanC.plot,newMeanC.plot, ncol=4,nrow 2)
